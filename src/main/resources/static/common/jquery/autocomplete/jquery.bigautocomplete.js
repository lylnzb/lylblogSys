		...
		this.init = function(){
			...
			$(document).bind('mousedown',function(event){
				...
				if((!($target.parents().andSelf().is('#bigAutocompleteContent'))) && (!$target.is(bigAutocomplete.currentInputText))){
					/*if($("#bigAutocompleteContent tr")){
						bigAutocomplete.currentInputText.val($("#bigAutocompleteContent tr.ct").html());
						var callback_ = bigAutocomplete.currentInputText.data("config").callback;
						if($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)){
							callback_($("#bigAutocompleteContent tr.ct").data("jsonData"));
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						$.ajax({ url: url, type: "POST",data: "keyword="+keyword_, dataType:"json",success: function(result){
							makeContAndShow(result.data);
						}});
					}
